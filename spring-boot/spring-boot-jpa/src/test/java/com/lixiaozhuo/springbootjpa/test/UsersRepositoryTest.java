...
import com.lixiaozhuo.springbootjpa.dao.*;
...
public class UsersRepositoryTest {
	...
	public void testCrudRepositoryFindOne() {
		Users users = this.usersRepositoryCrudRepository.findById(4).get();
		System.out.println(users);
	}
	...
	public void testCrudRepositoryDeleteById() {
		Users user = new Users();
		user.setId(4);
		this.usersRepositoryCrudRepository.delete(user);
	...
	public void testJpaSpecificationExecutor3() {
		...
		Specification<Users> spec = new Specification<Users>() {
			...
			/**
			 * Root<Users> root:查询对象的属性的封装。
			 * CriteriaQuery<?> query：封装了我们要执行的查询中的各个部分的信息，select  from order by 一般不使用
			 * CriteriaBuilder cb:查询条件的构造器。定义不同的查询条件
			 */
			@Override
			public Predicate toPredicate(Root<Users> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
